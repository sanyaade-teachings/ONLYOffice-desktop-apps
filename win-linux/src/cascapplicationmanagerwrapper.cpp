...
//                    open_opts.srctype = etLocalFile;
        ...
        if ( open_in_new_window ) {
            ...
            if (CTabPanel * panel = CEditorTools::createEditorPanel(open_opts, _app.m_pMainWindow->qtUnderlay())) {
...
bool CAscApplicationManagerWrapper::applySettings(const wstring& wstrjson)
{
    ...
    if( jerror.error == QJsonParseError::NoError ) {
        ...
        if (objRoot.contains("restart") && objRoot["restart"].toBool())
            res = CMessage::showMessage(mainWindow()->qtUnderlay(), tr("You must restart the application for the settings to take effect."),
